...
def insert_trigger(parser, original_code, code_lines, trigger, identifier, baits, position, multi_times,
    ...
    if mode in [-1, 0, 1]:
        if mode == 1:
            ...
            for idt_list in identifier_list:
                ...
                if idt_list[0] != "function_definition" and modify_idt in original_code:
                    continue
                elif idt_list[0] != "function_definition" and idt in keywords:
                    continue
                else:
                    idt_num = original_code.count(idt)
            ...
            if len(identifier) == 1 and identifier[0] == "function_definition":
                ...
                modify_code = re.sub(r'[^a-zA-Z]' + idt + r'[^a-zA-Z]', lambda x: x.group().replace(idt, modify_idt), original_code)
                ...
                for i in parameters_waiting_replace_list:
                    ...
                    modify_code = re.sub(r'[^a-zA-Z]' + idt + r'[^a-zA-Z]',
                                         lambda x: x.group().replace(idt, modify_idt), original_code)
